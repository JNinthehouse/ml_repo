...
class DenMune():
    ...
    def __init__(self,
        ...
        if train_data is None:
            raise Exception(
                "No data is provided. At least train data should be provided. Set train_data argmunt properly.")
        else:
        ...
        if train_data is not None and train_truth is None and test_truth is not None:
            raise Exception(
                "you should provide labels for your traing data to be allowed to work with test data. Set train_truth argmunt properly.")
        ...
        if train_data is not None and train_truth is not None and test_truth is not None and test_data is None:
            raise Exception(
                "Although labels of testing data is provided, the test data itself isnot. Set test_data argument properly.")
    ...
    def kd_NGT(self):
        ...
        if len(self.dp_dis) == 0:
            k = self.k_nearest
            nbrs = NearestNeighbors(n_neighbors=k + 1, algorithm='auto').fit(self.data)
            start = time.time()
            self.dp_dis = []
            distances, indices = nbrs.kneighbors(self.data)
            for i in range(self.dp_count):
                # Transforming the result to match the format of the original ngtpy result
                # We skip first distance from a point to itself
                result = [(idx, dist) for dist, idx in zip(distances[i][1:], indices[i][1:])]
                self.dp_dis.append(result)
    ...
    def plot_clusters(self, data_type=None, show_noise=False, show_plots=True):
        ...
        if show_plots:
            ...
            if self.prop_step:
                ...
                for c in colors:
                    if (c[0] + c[1] + c[
                        2]) > 0.0:  # outlier :: keep it away. Note that even outliers are -1, -2, it become in black after the previous step: color (0.0, 0.0, 0.0
